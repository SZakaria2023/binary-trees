...
// Function to create a BST from a table (array)
...
// Function to search for a node by value in the BST
...
// Function to create a copy of the BST
...
// Function to integrate two BSTs together
...
void render_text(SDL_Renderer* renderer, TTF_Font* font, const char* text, int x, int y) {
    SDL_Color color = {0, 0, 0}; // Black color for text
...
void SDL_RenderCircle(SDL_Renderer* renderer, int x, int y, int radius) {
    int offsetX, offsetY;
...
void draw_tree(SDL_Renderer* renderer, node* root, int x, int y, int offset, TTF_Font* font) {
    if (root != NULL) {
        char buffer[10];
        ...
        SDL_RenderCircle(renderer, x, y, 20);  // Draw filled circle (node)
        ...
        if (root->ls != NULL) {
            SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255); // Red for line
            SDL_RenderDrawLine(renderer, x, y, x - offset, y + 50); // Left line
            draw_tree(renderer, root->ls, x - offset, y + 50, offset / 2, font);
        }
        if (root->rs != NULL) {
            SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255); // Red for line
            SDL_RenderDrawLine(renderer, x, y, x + offset, y + 50); // Right line
            draw_tree(renderer, root->rs, x + offset, y + 50, offset / 2, font);
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        draw_tree(renderer, root, 400, 50, 100, font); // Draw the tree
